	...
	struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		if (video_app_network == WLAN0_INDEX) {
			choose_mark = WLAN0_MARK;

		} else if (video_app_network == WLAN1_INDEX) {
			choose_mark = WLAN1_MARK;
		}
	}
...
static u32 get_skb_mark_by_weight(void)
{
	...
	return oplus_sla_info[WLAN0_INDEX].mark;
}
...
static int dns_skb_need_sla(struct nf_conn *ct, struct sk_buff *skb)
{
	int ret = SLA_SKB_CONTINUE;
	struct iphdr *iph = NULL;
	u_int32_t dns_ct_mark = WLAN0_MARK;
	...
	if (INIT_APP_TYPE == get_app_type(ct)) {
		...
		*/
		if (oplus_sla_info[CELL_INDEX].if_up) {
			if (!oplus_sla_info[WLAN0_INDEX].if_up ||
...
static int oplus_sla_iface_changed(struct nlattr *nla)
{
	...
	if (index >= 0 && index < IFACE_NUM) {
		if (up) {
			...
			if (index == WLAN0_INDEX) {
				mark = WLAN0_MARK;
				oplus_sla_info[WLAN0_INDEX].if_up = 1;

			} else if (index == WLAN1_INDEX) {
				mark = WLAN1_MARK;
				oplus_sla_info[WLAN1_INDEX].if_up = 1;
...
static int oplus_sla_set_game_app_uid(struct nlattr *nla)
{
	...
	if (index < GAME_NUM) {
		...
		game_info[index].mark = WLAN0_MARK;
...
static int oplus_sla_change_game_network(struct nlattr *nla)
{
	...
	if (CELL_INDEX == network_index) {
		...
		game_info[game_index].mark = WLAN0_MARK;
	}
	sla_game_write_unlock();
