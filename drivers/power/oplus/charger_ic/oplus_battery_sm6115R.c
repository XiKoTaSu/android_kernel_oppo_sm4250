...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		chg_err("chg->qc_detect_time = %d, chg->qc_detach_time = %d chg->qc_break_unexpeactly = %d target 1000ms delta = %llu\n",
			chg->qc_detect_time, chg->qc_detach_time, chg->qc_break_unexpeactly, chg->qc_detect_time - chg->qc_detach_time);
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
		...
		if (oplus_vooc_get_fastchg_started() == true && oplus_vooc_get_fastchg_to_normal() == false	//plug out by normal
			...
			if (g_oplus_chip) {
				...
				g_oplus_chip->chargerid_volt_got = false;
				g_oplus_chip->charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
				g_oplus_chip->voocphy.fastchg_start = false;
...
/* Yichun.Chen  PSW.BSP.CHG  2019-04-08  for charge */
	...
	chg_type = opluschg_get_charger_type();
...
static bool opluschg_get_otg_switch(void)
{
	...
	return g_oplus_chip->ui_otg_switch;
}
